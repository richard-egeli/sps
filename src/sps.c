...
#define sps_error(msg)                                                         \
  do {                                                                         \
    fprintf(stderr, "sparse set assertion error in %s (%s:%d): %s\n",          \
            __func__, __FILE_NAME__, __LINE__, (msg));                         \
    abort();                                                                   \
  } while (0)
#else
...
void *sps_iter_next(sparse_set_iter_t *iter, uint16_t *index) {
  ...
  *index = iter->set->dense[iter->index];
  void *component = (char *)iter->set->component_data +
                    (iter->index * iter->set->component_size);
  iter->index++;
  return component;
}

sparse_set_iter_t sps_iter_new(sparse_set_t *set) {
  if (set == NULL || set->component_data == NULL) {
...
void sps_sort(sparse_set_t *set, sps_sort_func_t compare, void *context) {
  ...
  for (uint16_t i = 1; i < set->count; i++) {
    ...
    void *key_comp = (char *)set->component_data + (key * set->component_size);
    int j = i - 1;

    // Move elements that are greater than key to one position ahead
    while (j >= 0) {
      void *comp_j =
          (char *)set->component_data + (order[j] * set->component_size);
  ...
  for (uint16_t i = 0; i < set->count; i++) {
    ...
    memcpy((char *)temp_components + (i * set->component_size),
           (char *)set->component_data + (old_pos * set->component_size),
           set->component_size);
  ...
  memcpy(set->component_data, temp_components,
         set->count * set->component_size);
...
void *sps_add(sparse_set_t *set, uint16_t index, void *component) {
  ...
  if (set->component_data == NULL || set->component_size == 0) {
    sps_error("sparse set component is invalid");
    return NULL;
  }
  ...
  void *target =
      (char *)set->component_data + (set->count * set->component_size);
...
void sps_remove(sparse_set_t *set, uint16_t index) {
  ...
  void *last_comp =
      (char *)set->component_data + ((set->count - 1) * set->component_size);
  void *target =
      (char *)set->component_data + (set->sparse[index] * set->component_size);
...
void *sps_get(sparse_set_t *set, uint16_t index) {
  ...
  return (char *)set->component_data +
         (set->sparse[index] * set->component_size);
}

sparse_set_t *sps_new(size_t component_size) {
  sparse_set_t *sps = malloc(sizeof(*sps));
  if (sps == NULL) {
    sps_error("failed to allocate sparse set");
    return NULL;
  }

  sps->component_data = calloc(1, SPARSE_SET_MAX * component_size);
  if (sps->component_data == NULL) {
    sps_error("failed to allocate component data");
    free(sps);
    return NULL;
  }
